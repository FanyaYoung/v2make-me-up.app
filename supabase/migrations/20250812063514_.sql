-- Fix RLS policy performance issues by optimizing auth.uid() calls

-- Drop the conflicting policy first
DROP POLICY IF EXISTS "Allow all authenticated users" ON public.test_updated_at;

-- Update existing policies to wrap auth.uid() calls with (select auth.uid())
...
ALTER POLICY "System can insert activity" ON public.user_activity
WITH CHECK (true);
...
ALTER POLICY "System can manage analytics" ON public.user_analytics
USING (true)
WITH CHECK (true);
...
-- Fix admin policies for orders
ALTER POLICY "Admin can manage all orders" ON public.orders
USING (EXISTS (
  SELECT 1 FROM profiles 
  WHERE profiles.id = (select auth.uid()) 
  AND (profiles.preferences ->> 'role' = 'admin' OR profiles.preferences ->> 'is_admin' = 'true')
));

ALTER POLICY "Admin can manage all order items" ON public.order_items
USING (EXISTS (
  SELECT 1 FROM profiles 
  WHERE profiles.id = (select auth.uid()) 
  AND (profiles.preferences ->> 'role' = 'admin' OR profiles.preferences ->> 'is_admin' = 'true')
